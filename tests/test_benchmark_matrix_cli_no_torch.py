...
def test_stability_helpers_live_in_focused_modules():
    ...
    assert stability_report.format_stability_result_row(row).startswith("| run | ok |")


def test_oom_retry_continues_after_non_oom_retry_failure(monkeypatch):
    case_dir = _tmp_case("oom_retry_continue")
    args = Namespace(
        max_steps=1,
        warmup_steps=0,
        batch_size=4,
    ...
    assert [attempt[0] for attempt in attempts] == ["fsdp", "deepspeed_zero2"]
